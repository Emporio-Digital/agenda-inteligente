...
export default function BookingSystem({ tenantId, services, professionals, primaryColor }: BookingProps) {
  // Passos: 1=Serviço, 2=Pro, 3=DATA, 4=HORA, 5=FIM
  ...
  return (
    // ADICIONEI "text-zinc-900" AQUI EMBAIXO PARA FORÇAR LETRA ESCURA NO FUNDO BRANCO
    <div className="max-w-md mx-auto mt-6 bg-white p-6 rounded-xl shadow-lg border border-gray-100 text-zinc-900">
      ...
      {step === 1 && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          {services.map((service) => (
            <div key={service.id} onClick={() => handleServiceSelect(service)} className="border p-4 rounded-lg flex justify-between cursor-pointer hover:bg-gray-50 transition-all border-gray-200">
              <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {professionals.map((pro) => (
              <div key={pro.id} onClick={() => handleProfessionalSelect(pro)} className="border border-gray-200 p-4 rounded-lg cursor-pointer hover:bg-gray-50 text-center transition-all">
                <div className="w-16 h-16 rounded-full bg-gray-200 mx-auto mb-2 flex items-center justify-center text-2xl overflow-hidden">
                  {pro.photoUrl ? <img src={pro.photoUrl} alt={pro.name} /> : "✂️"}
                </div>
      ...
      {step === 3 && (
        <div className="space-y-4 animate-in fade-in slide-in-from-right-8 duration-300">
          ...
          <input 
            type="date" 
            className="w-full p-4 border-2 rounded-xl text-lg font-bold text-center text-black bg-white"
      ...
      {step === 5 && (
        <div className="animate-in fade-in slide-in-from-right-8 duration-300 text-center">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6 text-left text-sm space-y-1 text-black border border-gray-100">
          ...
          <div className="space-y-3">
            <input type="text" placeholder="Seu Nome" className="w-full p-3 border rounded-lg text-black bg-white border-gray-300" value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
            <input type="tel" placeholder="Seu WhatsApp" className="w-full p-3 border rounded-lg text-black bg-white border-gray-300" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} />
          </div>
      ...
      {step === 6 && (
        <div className="text-center animate-in zoom-in duration-500 py-8">
            ...
            <div className="bg-zinc-900 text-white p-6 rounded-xl mt-8 shadow-xl">
                <p className="font-semibold text-lg mb-2">Quer um site desse?</p>
                <a href="#" className="block w-full bg-white text-black font-bold py-3 rounded-lg hover:bg-gray-200 transition-colors">Criar Grátis</a>
            </div>
        </div>
      )}
    </div>
